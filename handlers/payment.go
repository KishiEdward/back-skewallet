...
func (h *PaymentHandler) Transfer(c *gin.Context) {
	...
	if req.Reference != "" {
		go func(ref string, amount float64) {
			webhookURL := "http://localhost:8080/v1/webhook/skewallet"
			...
			request, _ := http.NewRequest("POST", webhookURL, bytes.NewBuffer(payloadBytes))
			...
			if err != nil {
				log.Printf("[WEBHOOK ERROR] Gagal mengirim sinyal ke Skena ID: %v", err)
				return
			}
			defer resp.Body.Close()
			log.Printf("[WEBHOOK SUCCESS] Sinyal lunas terkirim untuk referensi %s, status Skena ID: %s", ref, resp.Status)
